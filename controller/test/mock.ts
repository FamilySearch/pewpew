...
import {
  ...
  ServiceInputTypes as SecretsServiceInputTypes,
  ServiceOutputTypes as SecretsServiceOutputTypes,
  TagResourceCommand
...
// Creating a shared one for this run
const DEFAULT_ENCRYPTION_KEY = crypto.randomBytes(SECRET_LENGTH);
...
export const UNIT_TEST_BUCKET_NAME: string = "unit-test-bucket";
export const UNIT_TEST_BUCKET_URL: string = `${UNIT_TEST_BUCKET_NAME}.s3.us-east-1.amazonaws.com`;
export const UNIT_TEST_KEYSPACE_PREFIX: string = "s3/pewpewcontroller-unittests-s3/";
...
export function mockListObject ({ filename = UNIT_TEST_FILENAME, folder = UNIT_TEST_KEY_PREFIX, lastModified = new Date() }: {
  filename?: string, folder?: string, lastModified?: Date
} = {}) {
...
export function mockUploadObjectError (message: string = "Shouldn't be called") {
  const mockedS3Instance: AwsStub<S3ServiceInputTypes, S3ServiceOutputTypes, S3ClientResolvedConfig> = mockS3();
  const error = new Error(message);
  mockedS3Instance.on(PutObjectCommand).rejectsOnce(error);
  mockedS3Instance.on(CreateMultipartUploadCommand).rejectsOnce(error);
  mockedS3Instance.on(CompleteMultipartUploadCommand).rejectsOnce(error);
}
...
let _mockedSecretsInstance: AwsStub<SecretsServiceInputTypes, SecretsServiceOutputTypes, SecretsManagerClientResolvedConfig> | undefined;
export function mockSecrets (): AwsStub<SecretsServiceInputTypes, SecretsServiceOutputTypes, SecretsManagerClientResolvedConfig> {
  log("mockSecrets enter", LogLevel.DEBUG);
  if (_mockedSecretsInstance !== undefined) {
    log("mockSecrets exists", LogLevel.DEBUG);
    return _mockedSecretsInstance;
  }
  // _mockedSecretsInstance = mockClient(SecretsClient);
  secretsConfig.secretsClient = undefined as any;
  initSecrets();
  _mockedSecretsInstance = mockClient(secretsConfig.secretsClient);
  log("mockSecrets created", LogLevel.DEBUG, { mockedSecretsInstance: _mockedSecretsInstance, secrets: secretsConfig.secretsClient });
  // Always mock DeleteSecretCommand so we don't accidentally call it behind the scenes. Don't expose the call like the others
  _mockedSecretsInstance.on(DeleteSecretCommand).resolves({ ARN: "mocked", Name: "mocked" } as DeleteSecretCommandOutput);
  // Always mock CreateSecretCommand
  _mockedSecretsInstance.on(CreateSecretCommand).resolves({ ARN: "mocked", Name: "mocked" } as CreateSecretCommandOutput);
  // Always mock TagResourceCommand
  _mockedSecretsInstance.on(TagResourceCommand).resolves({});
...
export function resetMockSecrets (): void {
  if (_mockedSecretsInstance !== undefined) {
    _mockedSecretsInstance.reset();
    secretsConfig.secretsClient = undefined as any;
    _mockedSecretsInstance = undefined;
    getGlobalSecretsConfig().encryptionKey = undefined;
    getGlobalSecretsConfig().openIdClientSecret = undefined;
  }
}

export function mockGetSecretValue (
  secretValue: string | Buffer = DEFAULT_ENCRYPTION_KEY
) {
  const mockedSecretsInstance: AwsStub<SecretsServiceInputTypes, SecretsServiceOutputTypes, SecretsManagerClientResolvedConfig> = mockSecrets();

  const mockedGetSecretValueCommandResult: Partial<GetSecretValueCommandOutput> = {
    Name: "testkey",
    ARN: "arn:aws:secretsmanager:us-east-1:unittests:testkey",
    SecretBinary: typeof secretValue !== "string" ? secretValue : undefined,
    SecretString: typeof secretValue === "string" ? secretValue : undefined,
    CreatedDate: new Date()
  };
  mockedSecretsInstance.on(GetSecretValueCommand).resolves(mockedGetSecretValueCommandResult);
  log("mockGetQueueAttributes", LogLevel.DEBUG, { mockedGetQueueAttributes: mockedGetSecretValueCommandResult, secrets: secretsConfig.secretsClient });
}
