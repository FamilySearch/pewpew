...
describe("config-wasm", () => {
  ...
  it("should throw error on non-string log_level", (done) => {
    try {
      config = new Config(integrationFile, varMap, 1);
      done(new Error("number should have failed"));
    } catch (error) {
      expect(`${error}`).to.include("Only strings are supported for log_level");
      done();
    }
  });
  ...
  it("should throw error on non-string log_level", (done) => {
    try {
      config = new Config(integrationFile, varMap, { level: "debug" });
      done(new Error("number should have failed"));
    } catch (error) {
      expect(`${error}`).to.include("Only strings are supported for log_level");
      done();
