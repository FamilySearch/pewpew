...
- PERF-4580 Fix `encode()` not escaping `&` or `+`
  - **Behavior change**: `encode(value, "percent-userinfo")` now also encodes `&` and `+`. Neither was encoded before, so a value containing an `&` silently split an `application/x-www-form-urlencoded` body or a query string into extra parameters, and a `+` was decoded as a space. This affects every existing caller of `"percent-userinfo"`; the output is now correctly escaped where it previously was not.
