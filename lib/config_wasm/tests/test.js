...
// import { Config } from "config-wasm";
...
const epochLoggerYaml = "epoch_logger.yaml";
...
describe("config-wasm", () => {
  ...
  before(async () => {
    try {
      ...
      epochLoggerFile = await readFile(joinPath(yamlPath, epochLoggerYaml));
    } catch (error) {
  ...
  it(epochLoggerYaml + " should pass with variables", (done) => {
    try {
      varMap.set("PORT","8081");
      config = new Config(epochLoggerFile, varMap);
      expect(config).to.not.equal(undefined);
      config.checkOk();
      expect(config.getBucketSize(), "getBucketSize").to.not.equal(undefined);
      expect(config.getBucketSize().toString(), "getBucketSize").to.equal("60");
